...
func TestBuildLPDGoldilocks(t *testing.T) {
	...
	c := gold.Consciousness()
	if c[0] < 0.8 {
		t.Fatalf("consciousness Acc keep %v", c)
	}
	d := trap.MemoryDensity()
	if d[0] != 0 || d[1] != 0 || d[2] != 0 {
		t.Fatalf("trap density radar must be origin: %v", d)
	}
	if gold.RelFast > 0.5 {
...
func TestBuildLPDTrapDoesNotSetThruPeak(t *testing.T) {
	...
	if bin.RelAcc >= lucy.LPDKeepFloor {
